...
{
	INT8 trueTemperature = 0;
	
	if ( infoByte == 0xFF )
		m_bushStatus = BUSH_STATUS::HEAT_SENS_ERR;
	else
	{
		trueTemperature = infoByte - TEMPERATURE_STAB;
		if ( ( m_bushStatus == BUSH_STATUS::OVERHEATED || m_bushStatus == BUSH_STATUS::HEAT_SENS_ERR ) && trueTemperature < MIN_OVERHEAT_TEMP )
